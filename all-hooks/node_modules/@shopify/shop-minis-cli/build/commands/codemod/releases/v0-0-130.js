import { readFile, writeFile } from 'node:fs/promises';
import path from 'node:path';
import { SemVer } from 'semver';
/**
 * Diff: https://github.com/Shopify/shop-minis-cli/compare/v0.0.127...v0.0.128
 */
async function applyUpgrade() {
    const manifestJsonPath = path.join(process.cwd(), 'src', 'manifest.json');
    const manifestJsonString = await readFile(manifestJsonPath, {
        encoding: 'utf-8',
    });
    const manifestJson = JSON.parse(manifestJsonString);
    delete manifestJson.partner_name;
    await writeFile(manifestJsonPath, `${JSON.stringify(manifestJson, null, 2)}\n`, // keep the new line at the end of the file
    {
        encoding: 'utf-8',
    });
}
const releaseNotes = ['Deprecate partner_name manifest field'];
const exports = {
    releaseNotes,
    applyUpgrade,
    version: new SemVer('0.0.130'),
};
export default exports;
//# sourceMappingURL=v0-0-130.js.map